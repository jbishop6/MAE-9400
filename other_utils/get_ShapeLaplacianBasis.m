function [evecs, evals] = get_ShapeLaplacianBasis(S,numEigs)
...
% W(W==-Inf) = 0;
try
    [evecs, evals] = eigs(S.Phi.W, S.Phi.A, numEigs, 1e-6);
catch
    % In case of trouble make the laplacian definite
    [evecs, evals] = eigs(S.Phi.W - 1e-8*speye(S.n), S.Phi.A, numEigs, 'sm');
end
evals = diag(evals);
if ~isreal(evecs)
